...
namespace Leopotam.EcsProto.Unity {
    public sealed class UnityModule : IProtoModule {
        ...
        public UnityModule (bool disableDebugSystems = default, string systemsName = default, bool bakeComponentsInName = true, string entityNameFormat = "D6") {
        ...
        public void Init (IProtoSystems systems) {
            ...
            if (!_disableDebugSystems) {
                systems.AddSystem (new ProtoSystemsDebugSystem (_systemsName));
                systems.AddSystem (new ProtoWorldDebugSystem (default, _bakeComponentsInName, _entityNameFormat));
                foreach (var kv in systems.NamedWorlds ()) {
                    systems.AddSystem (new ProtoWorldDebugSystem (kv.Key, _bakeComponentsInName, _entityNameFormat));
                }
            }
#endif
        }
